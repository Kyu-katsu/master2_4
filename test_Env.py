...
class DynamicObject:
    def __init__(self, id, dt):
        ...
        # (진우 수정) 속력 제한 0~0.5로 수정
        self.speed = np.random.uniform(0, 0.5) * dt
        self.direction = np.random.uniform(0, 2 * np.pi)
        # (진우 수정) 가속도도 dt 비례하게 변경
        self.ax = np.random.uniform(-0.5, 0.5) * dt
        self.ay = np.random.uniform(-0.5, 0.5) * dt
    ...
    def update(self, dt):
        # 1. 가속도 업데이트
        # (진우 수정) 속도 쪽에 있던 * dt 가속도 쪽으로 가져 왔음
        self.ax = np.clip(self.ax + np.random.uniform(-0.05, 0.05), -0.5, 0.5) * dt
        self.ay = np.clip(self.ay + np.random.uniform(-0.05, 0.05), -0.5, 0.5) * dt

        # 2. 속도 성분 계산 (m/s)
        # (진우 수정) 속도 쪽에 있던 * dt 가속도 쪽으로 가져 왔음
        vx = self.speed * np.cos(self.direction) + self.ax
        vy = self.speed * np.sin(self.direction) + self.ay

        # 3. 새 방향 계산
        target_direction = math.atan2(vy, vx)
        self.direction = 0.9 * self.direction + 0.1 * target_direction + np.random.normal(0, 0.05)

        # 4. 새 속력 계산 및 제한
        self.speed = np.sqrt(vx ** 2 + vy ** 2)
        self.speed = np.clip(self.speed, 0, 0.5)
